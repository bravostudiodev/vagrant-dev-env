...
Set-ItemProperty -Path $keyExplorerOpts -Name HideFileExt -Type "DWord" -Value 0 -Force | Out-Null
Write-Output "Show Run command in Start Menu" | Out-Default
Set-ItemProperty -Path $keyExplorerOpts -Name Start_ShowRun -Type "DWord" -Value 1 -Force | Out-Null
Write-Output "Show Administrative Tools in Start Menu" | Out-Default
Set-ItemProperty -Path $keyExplorerOpts -Name StartMenuAdminTools -Type "DWord" -Value 1 -Force | Out-Null

$keyConsoles = 'HKCU:\Console'
EnsureKey $keyConsoles
Write-Output "Enable Console Quick Edit mode" | Out-Default
Set-ItemProperty -Path $keyConsoles -Name QuickEdit -Type "DWord" -Value 1 -Force | Out-Null
